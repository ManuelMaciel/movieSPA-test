...
const MoviesContainer = () => {
  ...
  const getMovies = async () => {
    const resp = await AxiosClient.get(
      `/api/pelicula/?limit=${pagination}&offset=${offset}`
    );
            ...
            {indicator.prev === null ? null : (
              <li className="page-item">
                <button
                  className="page-link"
                  tabindex="-1"
                  onClick={() => setOffset(offset - pagination)}
                >
                  Previous
                </button>
              </li>
